...
class BaseDataLoader:
    ...
    async def _get_req(self, resource, params=None, session=None):
        req_url = self._base_url + resource
        if params is not None:
            self._logger.debug(f"GET: url={req_url}, pramas={params}")
            async with session.get(req_url, params=params) as response:  # Use async with for session
                return await response.json()
        else:
            self._logger.debug(f"GET: url={req_url}")
            async with session.get(req_url) as response:  # Use async with for session
                return await response.json()
